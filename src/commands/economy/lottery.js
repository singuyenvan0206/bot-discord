...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (action === 'buy' || action === 'b') {
            ...
            db.removeBalance(message.guild.id, message.author.id, cost);
            db.addLotteryTicket(message.guild.id, message.author.id, amount);
            db.addLotteryJackpot(message.guild.id, Math.floor(cost * 0.8)); // 80% price goes to jackpot
        ...
        const jackpot = db.getLotteryJackpot(message.guild.id);
        const tickets = db.getLotteryTickets(message.guild.id);
        ...
        const lastDraw = parseInt(db.getGuildSetting(message.guild.id, 'last_lottery_draw', '0'));
